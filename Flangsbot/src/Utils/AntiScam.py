...
async def AntiScam(message, bot, whitelist, muted_role, verified_role, logs_channel):
    ...
    if spam_counter > 1 and message.author.id not in whitelist:
        ...
        embd = discord.embeds.Embed(
            title='User got muted',
            description=f'{message.author.mention} got muted for spamming.'
        ).add_field(
            name='Reason',
            value='Phishing',
            inline=False
        ...
        await channel.send(f'Content of message: {message_content}')
